...
\begin{document}
	...
	\begin{frame}{The Missing Ingredient}
		...
		\only<3>{\begin{equation}
			\vec{P} = \mathcal{A}(\vec{y}_N)  
		\end{equation}}

		\only<4>{\begin{equation}
			\vec{P} = \begin{pmatrix} a_N(y_N^1) \\ a_N(y_N^2) \\ a_N(y_N^3) \\ \vdots \end{pmatrix}
		\end{equation}}

		\only<5>{\begin{equation}
			\begin{pmatrix} P_1 \\ P_2 \\ P_3 \\ \vdots \end{pmatrix} = \begin{pmatrix} a_N(\vec{w}_N^1 \cdot \tilde{x}_{N-1}) \\ a_N(\vec{w}_N^2 \cdot \tilde{x}_{N-1}) \\ a_N(\vec{w}_N^3 \cdot \tilde{x}_{N-1}) \\ \vdots \end{pmatrix}
		\end{equation}}

	\end{frame}

	\begin{frame}{Getting the Gradients}

		\pause Then:
		\begin{equation}
			\pdiv{P_i}{y_N^i} = d_N(y_N^i)  
		\end{equation}
		\pause Which means:
		\begin{align}
			\pdiv{C}{y_N^i} & = \pdiv{C}{P_i} \times \pdiv{P_i}{y_N^i}
			\\
			& = d_N(y_N^i)\pdiv{C}{P_i} 
		\end{align}
